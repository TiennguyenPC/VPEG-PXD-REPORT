...
if (!code.includes("const hStr = String(headers[colIndex]).trim().toUpperCase();")) {
  // Fix 1: appendRow
  code = code.replace(
    "newRow[colIndex] = val;",
...
const matchPattern = /case 'dashboard-bundle':[\s\S]*?case 'projects':/;
...
code = code.replace(matchPattern, newDashboardCode);
